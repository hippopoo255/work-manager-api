resource "aws_cognito_user_pool" "admin" {
  ...
  # email_verification_subject = "【${local.pj_name_kana}】検証コード"
  ...
  verification_message_template {
    ...
    email_subject        = "【${local.pj_name_kana}】検証コード"
  }
...
resource "aws_cognito_user_pool_client" "admin" {
  name         = "${local.pj_name_camel}ClientAdmin"
...
resource "aws_cognito_identity_pool" "admin" {
  identity_pool_name               = "${local.pj_name_kebab}-adminpool-${data.aws_default_tags.this.tags.Env}"
  allow_unauthenticated_identities = false

  cognito_identity_providers {
    client_id               = aws_cognito_user_pool_client.admin.id
    provider_name           = "cognito-idp.${local.cognito_region}.amazonaws.com/${aws_cognito_user_pool.admin.id}"
...
resource "null_resource" "test_user_admin_confirmed" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      COGNITO_USERNAME    = aws_cognito_user.test_user_admin.username
